...
export default function PhotosUploader({ addedPhotos, setAddedPhotos }) {
  ...
  function selectAsMainPhoto(e, filename) {
    e.preventDefault();
    setAddedPhotos([filename, ...addedPhotos.filter((name) => name !== filename)]);
  }
  ...
  return (
    <>
      ...
      <div className="flex gap-2">
        <input value={photoLink} onChange={(e) => setPhotoLink(e.target.value)} type="text" placeholder="Add using link" />
        <button onClick={addPhotoLink} className="bg-gray-200 grow px-4 rounded-2xl">
          Add&nbsp;Photo
        </button>
      </div>
